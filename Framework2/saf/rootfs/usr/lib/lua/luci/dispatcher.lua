...
function authenticator.htmlauth(validator, accs, default)
	...
	if user then
		...
		if failtimes ~= 0 and failtimes % 3 == 0 and rsttime <= 60 then
			luci.template.render("login", {duser=default, fuser=user, ta=ta, failtimes=failtimes, rsttime=(60 - rsttime), mutex=0})
			return false
		end
		...
		elseif pass and validator(user, pass) then
			...
			if login and login == 1 and curlan then			
				if curlan ~= lanhost then
					luci.template.render("login", {duser=default, fuser=user, ta=ta, failtimes=failtimes, rsttime=0, mutex=1})
					return false
				else
	...
	if regStatus ~= 200 and regStatus ~= 402 then
		...
		if hostaddr and (hostaddr ~= addrs.LANIP) and 
		   (hostaddr:lower() ~= "[fe80::1]") and 
		   (hostaddr ~= addrs.WANIP) and 
		   (hostaddr ~= wan6addr) then
			luci.template.render("register")                                        
			return false
		end
	end
	luci.template.render("login", {duser=default, fuser=user, ta=ta, failtimes=failtimes, rsttime=60, mutex=0})	
	return false
