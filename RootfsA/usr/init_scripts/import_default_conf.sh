...
if [ $RCOUNT -le 3 ]; then
	...
	UUIDCOUNT=0
	FLOWUUIDSTR=
	for ofcmd in $(cat /usr/init_scripts/default_flows.conf)
	do
		echo "$ofcmd"
...
if [ $RCOUNT -le 3 ]; then
	...
	if [ -n "$ssid" ]; then
		...
		TRANSACT_STR='["Open_vSwitch",{"op":"insert","table":"WLAN_RADIO","row":{"radio_enable":1,"radio_band":"2.4G","radio_mode":"802.11b/g/n","channel":0,"bandwidth":1,"power":1}}'
	...
	if [ -n "$ssid11ac" ]; then
		...
		TRANSACT_STR='["Open_vSwitch",{"op":"insert","table":"WLAN_RADIO","row":{"radio_enable":1,"radio_band":"5G","radio_mode":"802.11a/n","channel":0,"bandwidth":1,"power":1}}'
